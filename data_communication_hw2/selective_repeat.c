...
int main(int argc, char **argv)
{
    ...
    if (argc != 6) {
        ...
        printf("./selective_repeat [W] [lambda] [t_pk] [p] [a]\n");
        return -1;
    }
...
{
    DataQue *ptr, *prev_ptr;
    AckQue *aptr;
    // deque <- TransitQ
    ptr = TransitQ_front;
    if(ptr->sn == sn) {
        //printf("successfully transmitted 1: %ld\n", sn);
        // deque <- TransitQ_front
        TransitQ_front = TransitQ_front->link;
        if (TransitQ_front == NULL)
            TransitQ_rear = NULL;
        free(ptr);
        // no error occured before -> window size +1
        if (buf_num == 0) transit_pknum--;
        // Re-transmitted frame after error -> window size +(the # of bufferd frame + 1)
        else {
            transit_pknum = transit_pknum - (buf_num + 1);
            buf_num = 0;
        }
    }
    else if (ptr->sn < sn) {
        // after error occured,
        // find the DataQue and remove from the middle of TransitQ
        // window size -> no change
        prev_ptr = ptr;
        ptr = ptr->link;
        while(ptr) {
            if (ptr->sn == sn) {
                //printf("successfully transmitted 2: %ld\n", sn);
                prev_ptr->link = ptr->link;
                if (prev_ptr->link == NULL) TransitQ_rear = prev_ptr;
                free(ptr);
                buf_num++;
            }
            prev_ptr = ptr;
...
void re_transmit(void)
{
    ...
    DataQue *ptr;
    ptr = TransitQ_front;
    // remove TransitQ_front
    TransitQ_front = TransitQ_front->link;
    if (TransitQ_front == NULL) TransitQ_rear = NULL;
    // and put it on WQ_front
    ptr->link = WQ_front;
    if (WQ_front == NULL) WQ_rear = ptr;
    WQ_front = ptr;
